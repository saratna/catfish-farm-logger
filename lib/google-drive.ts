...
function weeklyReportHtml(report: WeeklyReportExport) {
  const rows = report.tankSummaries.map((tank) => `<tr><td>${escapeHtml(tank.tankName)}</td><td>${tank.inspections}</td><td>${tank.feedKg.toFixed(1)} kg</td><td>${tank.growthRecords}</td><td>${escapeHtml(tank.healthSeverity)} (${tank.diseaseAlerts})</td><td>${peso(tank.cost)}</td><td>${peso(tank.sales)}</td><td>${peso(tank.grossProfit)}</td></tr>`).join("");
...
export async function uploadFarmExportToGoogleDrive(exportPayload: DriveExport) {
  ...
  for (const tankExport of exportPayload.tanks) {
    ...
    const files = [
      ...
      { name: "health-snapshot.json", value: tankExport.healthSnapshot },
